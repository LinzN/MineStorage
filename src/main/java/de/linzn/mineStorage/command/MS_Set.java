...
public class MS_Set extends SYS_Command_API {

	public MS_Set(CMD_Command_Provider handler, String specialName) {
	...
	public boolean execute(final CommandSender sender, final String[] args) {
		if (sender.hasPermission("minestorage.admin.cmd.set")) {
			...
			task.executorServiceCommands.submit(new Runnable() {
				...
				public void run() {
					if (args.length == 3) {
						...
						String uuid = Bukkit.getOfflinePlayer(args[1]).toString();
						int amount = 0;
						try {
						...
						MS_Set.this.setDBUUIDExp(uuid, amount, p, args[1]);
						return;
					}
